...
import 'package:demoapp/src/services/database.dart';
import 'dart:math';
import 'package:demoapp/src/utils/theme_colors.dart';
import 'package:demoapp/src/utils/constants.dart';
...
class _DayViewState extends State<DayViewScreen> {
  ...
  //       createdOn: _value,
  //       grams: 0);
  //   databaseService.getFoodTrackData();
  // }
  ...
  bool checkFormValid() {
    if (addFoodTrack.food_name.isEmpty) {
      return false;
    }
    if (addFoodTrack.calories <= 0 ||
        addFoodTrack.carbs <= 0 ||
    ...
    return Form(
      ...
      child: Column(children: [
        ...
        TextFormField(
          ...
          onEditingComplete: () {
            final currentValue = addFoodTrack.food_name;
        ...
        //       ScaffoldMessenger.of(context).showSnackBar(
        //         SnackBar(
        //           content: Text("Please enter numeric values only"),
        //           backgroundColor: Colors.red,
        //         ),
        //       );
        //     } else {
        ...
        //   },
        //   onEditingComplete: () {
        //     final currentValue = addFoodTrack.calories.toString();
        //     if (currentValue.isEmpty) {
        //       ScaffoldMessenger.of(context).showSnackBar(
        //         SnackBar(
        //           content: Text("Please enter a calorie amount"),
        //           backgroundColor: Colors.red,
        //         ),
        //       );
        //     }
          ...
          onChanged: (value) {
            if (!RegExp(r'^[0-9]+$').hasMatch(value)) {
              if (value.isNotEmpty && RegExp(r'^[a-zA-Z]+$').hasMatch(value)) {
                addFoodTrack.calories = 0; // or perform any appropriate action
              } else {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text("Please enter numeric values only"),
        ...
        TextFormField(
          ...
          onChanged: (value) {
            if (value.isEmpty) {
              return; // Skip validation if value is empty
            }
          ...
          onEditingComplete: () {
            final currentValue = addFoodTrack.carbs.toString();
            if (currentValue.isEmpty) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text("Please enter a carbs amount"),
                  backgroundColor: Colors.red,
                ),
              );
            }
...
class FoodTrackTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ExpansionTile(
      backgroundColor: Color.fromARGB(255, 176, 91, 196),
